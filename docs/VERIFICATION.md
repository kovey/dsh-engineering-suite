...
最近一次结果：**9 个包 + 1 个根集成测试，380 个测试，379 通过 / 0 失败 / 1 跳过**（另有真机 harness：8 个插件全部 `applied`，E2E GREEN）
...
| 包 | 测试数 | 覆盖重点 |
|---|---|---|
| `dsh-eng-core` | 65 | mission 工件持久化（含损坏文件读回 `undefined`、同秒 id 不冲突）、规格章节重复标题仍能解析（真机发现的既有 bug）、spec/测试设计 markdown 往返解析（含无前导竖线 / 全角竖线 / 无表头 / 标题串行）、无 shell 的命令执行、超时 / 抢占 / 进程组回收 |
