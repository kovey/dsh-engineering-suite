...
### 3.5 `dsh-impact-gate`：变更影响分析（改这里会影响谁）

**职责**：把"这次改动影响哪些文件、该跑哪些测试"从模型猜测变成**确定性事实**。

**实现要点**：
- 由 `git diff -U0`（或显式 paths）得到变更文件与新增行范围，再用导入图算**反向传递闭包**；
- 测试选择分四级证据：`changed`（测试自身被改）> `imports-changed`（导入了变更文件，带距离）
  > `same-package`（同目录，Go 这类不互相 import 的生态）> `name-match`（同名）；
- 风险分级用固定阈值（扇入 ≥8 / 影响面 ≥25 / 导出面 ≥15 → high；无测试覆盖 → high），阈值即门禁口径；
- **最小回归集是"必要"不是"充分"**：接口分派、依赖注入、反射、字符串查表都不可静态判定，
  报告里明确列出这些盲区；只有宿主显式配置 `fullTestCommand` 才做"省了多少"的对照。

### 3.6 `dsh-coverage-gate`：测试有效性（覆盖率 / 增量覆盖 / flaky）

**职责**：其他门禁问"测试过了吗"，它问"测试有用吗"。

**实现要点**：
- 解析 lcov / cobertura / go-cover / istanbul-json 四种报告（各含畸形输入拒绝），按阈值判总分与**新增行覆盖率**；
- 报告没插桩的行单独列出、**不参与比率**（绝不算 0% 或 100%）；非 git 工作区里配了增量阈值 → BLOCK（无法判定就不放行）；
- flaky 检测：有界重复执行，按用例名归因；runner 不报名字时如实降级为"运行级结论"；
- 门禁记录 `source: dsh-coverage-gate`，交付侧可要求它（与质量门禁同样的"最新且不早于证据"语义）。

### 3.7 `dsh-supply-chain-gate`：供应链与密钥（能不能上生产的硬门槛）

**实现要点**：
- `secret_scan` 默认只扫**本次改动新增的行**（"这次改动有没有引入"），11 条已知形状规则 + 熵检测 + 仓库自有 allowlist；
- **严重度分级**：只有已知凭据形状阻断；熵检测按上下文降为建议级——真机实测（`golang/im` 120 文件）
  初版"任何命中即阻断"会产生 461 条命中（几乎全是文档里的 base64），那种门禁会被立刻关掉；
- `dependency_audit`：清单 diff 找出新增依赖 → **人工批准**（拒绝/无通道/锁文件孤儿变更 → BLOCK）；
  宿主配置的审计命令按 argv 执行、按 severity 阻断；报告无法解析 → BLOCK（不是"没问题"）；
- 脱敏：报告与工件里只出现前 4 + 后 2 位与 sha256 指纹，**原始值永不落盘**。

### 3.8 `dsh-evidence-gate`：证据与交付层
