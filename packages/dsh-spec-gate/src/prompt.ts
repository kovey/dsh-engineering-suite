...
export function sectionText(config: SpecGateConfig, projectFile?: string): string {
    ...
    return [
        ...
        '规格有变更时，**单条增删改**用 `spec_amend`（`part` + `action` + `target`）：',
        '- `add`：新增一条需求/验收标准，分配新编号（不会打乱已有编号）；',
        '- `update`：按编号改写，编号保持不变——用例引用的是编号，改写不会让覆盖关系错位；',
        '- `remove`：删除，编号进入"已作废"（**永不复用**）；若该验收标准被测试用例引用会被**拒绝**，',
        '  除非 `cascade: true`（连同用例一起移除，并要求重新评审测试设计）。',
        '整篇重写（标题/背景/边界/测试设计章节）仍走 `spec_create`：它会按文本保留未变条目的编号，',
