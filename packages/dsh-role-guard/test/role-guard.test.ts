...
import { MissionStoreRegistry, renderSpecMarkdown } from 'dsh-eng-core'
...
test('a workspace role file is loaded and can override a builtin role', async () => {
    ...
    assert.match(auditor, /deepseek\/deepseek-chat/)
})
...
test('a model route in the role file becomes agentOptions', async () => {
    ...
    fs.writeFileSync(
        ...
        ['---', 'id: cheap', 'name: 便宜模型', 'mode: write', 'model: deepseek/deepseek-chat', 'tools:', '  - read', '---', '用便宜模型干活。', ''].join('\n'),
    )
    const fake = host(cwd)
    await fake.runTool('team_delegate', { role: 'cheap', task: 't' })
    const start = fake.subagentStarts[0] as { request: Record<string, unknown> }
    assert.deepEqual(start.request['agentOptions'], { provider: 'deepseek', model: 'deepseek-chat' })
})
...
test('an inline role config supports the same route fields as a role file (regression)', async () => {
    ...
    await fake.runTool('team_delegate', { role: 'cheap', task: 't' })
    const start = fake.subagentStarts[0] as { request: Record<string, unknown> }
    assert.deepEqual(start.request['agentOptions'], { provider: 'deepseek', model: 'deepseek-chat', reasoningEffort: 'low', maxTokens: 4096 })
})
